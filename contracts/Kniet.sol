...
/**
 * @title Kniet
 * @notice Platform credit system for the Kniet token voting & promotion dApp.
 *         Users purchase Kniet credits with ETH. Credits can be spent to vote
 ...
 */
contract Kniet is ReentrancyGuard, Ownable, Pausable {

    // ─── State ────────────────────────────────────────────────────────────

    /// @notice Kniet credit balance per address
    ...
    constructor(address payable _treasury) Ownable(msg.sender) {
        require(_treasury != address(0), "Kniet: zero treasury");
        treasury = _treasury;
    }
    ...
    /**
     * @notice Purchase Kniet credits by sending one of the accepted ETH amounts.
    ...
    function buyCredits() external payable nonReentrant whenNotPaused {
        ...
        require(reward > 0, "Kniet: invalid ETH amount");
    ...
    function buyRubyTier() external payable nonReentrant whenNotPaused {
        require(msg.value == rubyTierPrice, "Kniet: incorrect ruby price");
    ...
    function buyDiamondTier() external payable nonReentrant whenNotPaused {
        require(msg.value == diamondTierPrice, "Kniet: incorrect diamond price");
    ...
    function spendCredits(uint256 amount, string calldata reason) external nonReentrant {
        require(credits[msg.sender] >= amount, "Kniet: insufficient credits");
    ...
    function setTreasury(address payable _treasury) external onlyOwner {
        require(_treasury != address(0), "Kniet: zero address");
    ...
    function setCreditRate(uint256 index, uint256 ethAmount, uint256 creditsGiven) external onlyOwner {
        require(index < 9, "Kniet: index out of range");
    ...
    function withdraw() external nonReentrant onlyOwner {
        ...
        require(balance > 0, "Kniet: nothing to withdraw");
        (bool ok, ) = treasury.call{value: balance}("");
        require(ok, "Kniet: transfer failed");
